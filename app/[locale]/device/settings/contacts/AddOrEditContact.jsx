...
import { JuJiuLayer, ButtonLink, JuJiuInformation } from '@/jujiu-ui-components/core/core-ui';
import { JuJiuItem, JuJiuItemButton, JuJiuItemLink } from '@/jujiu-ui-components/core/core-item';
	...
	return (
		<>
			...
			<JuJiuItem label='出生日期'>
				<Box width='small'>
					<DateInput format='yyyy/mm/dd' value={new Date().toISOString()} onChange={({ value }) => {}} />
				</Box>
			</JuJiuItem>
			{noAccount && (
				<>
					<JuJiuItemButton label='绑定账号' onClick={() => setOpenAddAccound(true)} />
					{openAddAccount && (
						<JuJiuLayer onClickOutside={() => setOpenAddAccound(false)}>
			...
			{openAddContact && (
				<JuJiuLayer onClickOutside={() => setOpenAddContact(false)}>
					<Heading level={3} alignSelf='center' margin='none'>
						自定义关系
					</Heading>
					<TextInput value='堂哥' />
			...
			{openAddFace && (
				<JuJiuLayer onClickOutside={() => setOpenAddFace(false)}>
					<Heading level={3} alignSelf='center' margin='none'>
						选择录入方式
					</Heading>
